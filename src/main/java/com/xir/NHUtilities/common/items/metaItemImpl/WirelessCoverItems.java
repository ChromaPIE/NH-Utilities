    ...
    public void loadMetaItem() {
        ...
        for (int i = 0; i < 4; i++) {
            ...
            for (int j1 = 0; j1 < dynamoList.size(); j1++) {
                ...
                final int voltage = (int) GTValues.V[j1 + 1];
                final int amperage = 2 << ac[i];
                CoverRegistry.registerCover(
                    dynamoValue.get(1),
                    tex[i],
                    (context) -> new WirelessCovers.CoverWirelessDynamo(context, voltage, amperage)
                );
            }
            ...
            for (int j2 = 0; j2 < energyList.size(); j2++) {
                ...
                energyValue.set(
                    addMetaItem(
                        ...
                        new String[] {
                            CommonUtil.trans("tooltip.amp.info") + " " + formatNumbers(2 << ac[i]) + " A" }));
                final int voltage = (int) GTValues.V[j2 + 1];
                final int amperage = 2 << ac[i];
                CoverRegistry.registerCover(
                    energyValue.get(1),
                    tex[i],
                    (context) -> new WirelessCovers.CoverWirelessEnergy(context, voltage, amperage)
                );
            }
        }
